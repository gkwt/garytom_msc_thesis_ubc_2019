...
The success of an \ac{STML} experiment is dependent on many factors, such as the material of the tip and sample, the geometry of the tip, the quantum efficiency of the molecule, and the decoupling between molecule and metal substrate substrate.

For strong luminescence from tunnelling electrons, the tip and sample should be composed of a material with low dielectric loss, or a small imaginary component to its dielectric function. In \ac{STM}, the tip and sample must also be conductive, so the material should be a noble metal such as Ag, Au, or Pt. Low dielectric loss means that electric field can more easily penetrate the material, and the conduction electrons in the metal can move more freely within the bulk, allowing for stronger plasmonic response. Oxide layers or other contaminants can quench emissions from the tip-sample junction.

The tip geometry is crucial to maximizing signal-to-noise ratio in \ac{STML}. The \FIXME{unfinished}
